...
let trsc_change_manager(contr, from, new_ : Betting parameter_of contract * address * address) =
  let () = Test.set_source from in
  let result : test_exec_result = Test.transfer_to_contract contr (ChangeManager new_) 0mutez in
  result

let trsc_change_manager_success(contr, from, new_ : Betting parameter_of contract * address * address) =
  Assert.tx_success (trsc_change_manager(contr, from, new_))

let trsc_change_oracle_address(contr, from, new_ : Betting parameter_of contract * address * address) =
  let () = Test.set_source from in
  let result : test_exec_result = Test.transfer_to_contract contr (ChangeOracleAddress new_) 0mutez in
  result

let trsc_change_oracle_address_success(contr, from, new_ : Betting parameter_of contract * address * address) =
  Assert.tx_success (trsc_change_oracle_address(contr, from, new_))

let trsc_switch_pauseBetting(contr, from : Betting parameter_of contract * address) =
  let () = Test.set_source from in
  let result : test_exec_result = Test.transfer_to_contract contr (SwitchPauseBetting) 0mutez in
  result

let trsc_switch_pauseBetting_success(contr, from : Betting parameter_of contract * address) =
  Assert.tx_success (trsc_switch_pauseBetting(contr, from))

let trsc_switch_pauseEventCreation(contr, from : Betting parameter_of contract * address) =
  let () = Test.set_source from in
  let result : test_exec_result = Test.transfer_to_contract contr (SwitchPauseEventCreation) 0mutez in
  result

let trsc_switch_pauseEventCreation_success(contr, from : Betting parameter_of contract * address) =
  Assert.tx_success (trsc_switch_pauseEventCreation(contr, from))

let trsc_add_event(contr, from, event : Betting parameter_of contract * address * Types.add_event_parameter) =
  let () = Test.set_source from in
  let result : test_exec_result = Test.transfer_to_contract contr (AddEvent event) 0mutez in
  result

let trsc_add_event_success(contr, from, event : Betting parameter_of contract * address * Types.add_event_parameter) =
  Assert.tx_success (trsc_add_event(contr, from, event))

let trsc_update_event(contr, from, event_num, event : Betting parameter_of contract * address * nat * Types.event_type) =
  let () = Test.set_source from in
...
let trsc_update_event_success(contr, from, event_num, event : Betting parameter_of contract * address * nat * Types.event_type) =
  Assert.tx_success (trsc_update_event(contr, from, event_num, event))

let trsc_get_event(contr, from, cbk_addr, event_num : Betting parameter_of contract * address * address * nat) =
    let () = Test.set_source from in
...
let trsc_get_event_success(contr, from, cbk_addr, event_num : Betting parameter_of contract * address * address * nat) =
  Assert.tx_success (trsc_get_event(contr, from, cbk_addr, event_num))

let trsc_add_bet(contr, from, pRequestedEventID, pTeamOneBet, pBetAmount : Betting parameter_of contract * address * nat * bool * tez) =
  let () = Test.set_source from in
...
let trsc_add_bet_success(contr, from, pRequestedEventID, pTeamOneBet, pBetAmount : Betting parameter_of contract * address * nat * bool * tez) =
  Assert.tx_success (trsc_add_bet(contr, from, pRequestedEventID, pTeamOneBet, pBetAmount))

let trsc_finalize_bet(contr, from, pRequestedEventID : Betting parameter_of contract * address * nat) =
  let () = Test.set_source from in
  let result_tx = Test.transfer_to_contract contr (FinalizeBet pRequestedEventID) 0mutez in
  result_tx

let trsc_finalize_bet_success(contr, from, pRequestedEventID : Betting parameter_of contract * address * nat) =
